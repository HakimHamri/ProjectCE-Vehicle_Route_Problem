...
if uploaded_file:
    ...
    mu = st.sidebar.number_input("Parent population (mu)", 20, min_value=1)
    lam = st.sidebar.number_input("Offspring population (lambda)", 200, min_value=1)
    generations = st.sidebar.number_input("Generations", 200, min_value=1)
    ...
    if st.button("Run Evolution Strategy"):
        ...
        for i, r in enumerate(best_routes): st.write(f"Route {i+1}: Depot -> {' -> '.join(map(str,r))} -> Depot")

        # Convergence plot
        st.subheader("Convergence Plot")
        fig, ax = plt.subplots()
        ax.plot(history); ax.set_xlabel("Generation"); ax.set_ylabel("Best Distance")
        st.pyplot(fig)
